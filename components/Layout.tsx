...
const NavItem: React.FC<{ to: string; icon: React.ReactNode; label: string; active: boolean }> = ({ to, icon, label, active }) => (
  ...
  >
    {icon}
    <span className="mt-1 truncate max-w-full px-1">{label}</span>
  </Link>
);
...
export const Layout: React.FC<LayoutProps> = ({ children, isOnline, isSyncing, user }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 pt-safe">
      ...
      <nav className="bg-white border-t border-gray-200 fixed bottom-0 w-full z-20 flex justify-between items-stretch h-16 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] pb-safe">
        <NavItem 
          to="/" 
